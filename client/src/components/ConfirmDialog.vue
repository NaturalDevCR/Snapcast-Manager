            ...
            <DialogPanel class="relative transform overflow-hidden rounded-2xl bg-slate-800 border border-slate-700 p-6 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
              <div class="sm:flex sm:items-start">
                <div :class="[
                  ...
                  type === 'danger' ? 'bg-red-900/30 text-red-500' : 'bg-amber-900/30 text-amber-500'
                ]">
              ...
              <div class="mt-6 sm:mt-6 sm:flex sm:flex-row-reverse gap-3">
                <button
                  type="button"
                  :class="[
                    'inline-flex w-full justify-center rounded-xl px-4 py-2.5 text-sm font-semibold text-white shadow-sm sm:w-auto transition-all',
...
const props = defineProps({
  ...
  type: { type: String, default: 'warning' } // 'warning' or 'danger'
});

const emit = defineEmits(['update:modelValue', 'confirm']);

const icon = computed(() => {
  return props.type === 'danger' ? ExclamationTriangleIcon : QuestionMarkCircleIcon;
});
